// Here we are finding minima of unimodal function concave upward or convex function...
#include <bits/stdc++.h>
using namespace std;
...
// Example unimodal concave upward function (quadratic function) 
int f(int x) 
{ 
...
int ternary_search(int lo, int hi)
{
    ...
    int ans = INT_MAX;
    // Iterate from lo to hi, since hi-lo < 3 hogaya hai ab so minimum value toh , lo to hi
    //ke beech hi hoga so we can iterate from lo to hi to get the minimum value...
    for (int i = lo; i <= hi; i++)
    {
        ans = min(ans, f(i));
    }    
